...
```{r}
n_images <- list.files("outputs/stitched") |> length()
n_ocr <- ocr$panoid |> unique() |> length()
n_original_phrases <- read_csv("outputs/ocr.csv") |> 
  nrow()

n_current_phrases <- nrow(ocr)

tribble(
  ~Statistic, ~Value,
  "Number of scraped images for bounding box", n_images,
  "Number processed via OCR so far", n_ocr,
  "Total number of phrases identified", n_original_phrases,
  "Total number of phrases after filtering", n_current_phrases
) |> 
  kable()
...
ocr |> 
  st_drop_geometry() |> 
  group_by(language = lang_long) |> 
  summarize(
    `Recognized phrases` = n(),
    `Average length of phrase (characters)` = mean(nchar(text)),
